...
import Image from "next/image";
import Link from "next/link";
import BlogFAQClient from "../../components/BlogFAQ";
import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "APPortal Uploads :: Blog",
  description: "Detailed blog post with FAQs and resources.",
};
  ...
  return (
    <main className="container mx-auto px-6 py-10">
      <article>
        ...
        {blog.bannerImage && (
          <div className="flex justify-center mb-8">
            <div className="w-full max-w-[1100px] h-[200px] sm:h-[300px] md:h-[400px] relative">
              <Image
        ...
        {/* ✅ Blog Content */}
        {blog.blogData.map((section, index) => (
          <section key={index} className="mb-8 max-w-5xl mx-auto">
            {section.subTitle && (
              <h2 className="text-2xl font-semibold mb-3">{section.subTitle}</h2>
            )}
            {section.paragraphs?.map((p, i) => (
              <p key={i} className="text-gray-700 mb-2 text-justify">
                {p}
              </p>
            ))}
          </section>
