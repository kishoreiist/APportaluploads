...
export default function LeadershipPage() {
  ...
  return (
    <main className={`min-h-screen bg-gray-50 text-gray-800 ${selectedLeader ? 'overflow-hidden h-screen' : ''}`}>
      <Navbar />
      ...
      {ceo && (
        <section className="max-w-3xl mx-auto px-4 sm:px-6 py-12 text-center">
          <div
            ...
            onClick={() => setSelectedLeader(ceo)}
          >
            <div className="w-32 h-44 sm:w-40 sm:h-56 mb-4 flex items-center justify-center overflow-hidden rounded-xl mx-auto">
              <Image
                src={ceo.image}
                alt={`Portrait of ${ceo.name}`}
                width={160}
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
        <div className="flex flex-wrap justify-center gap-8">
          {others.map((leader) => (
            <div
              key={leader.name}
              className="bg-white shadow-lg rounded-2xl overflow-hidden text-center p-6 w-full sm:w-[45%] md:w-[30%] cursor-pointer hover:shadow-2xl transition"
              onClick={() => setSelectedLeader(leader)}
            >
              <div className="w-28 h-40 sm:w-36 sm:h-48 mb-4 flex items-center justify-center overflow-hidden rounded-xl mx-auto">
                <Image
                  src={leader.image}
                  alt={`Portrait of ${leader.name}`}
                  width={140}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-[95vw] sm:max-w-3xl max-h-[90vh] overflow-y-auto p-6 relative">
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="text-left">
                <h2 className="text-xl sm:text-2xl font-bold text-indigo-700">{selectedLeader.name}</h2>
                <p className="text-gray-600 mb-4">{selectedLeader.role}</p>
                <p className="text-gray-700 whitespace-pre-line leading-relaxed">
                  {selectedLeader.description}
                </p>
